...
%% Setting of paths
path_to_database = '/home/jhonygiraldoz/SBMI_dataset/';
...
%%
challenges = {'SBMI_dataset'};
sampling_density = [0.001,0.005,0.1,0.02:0.02:0.2];
repetitions = 5;
%% Metrics
average_recall = cell(length(challenges),1);
average_specficity = cell(length(challenges),1);
average_FPR = cell(length(challenges),1);
average_FNR = cell(length(challenges),1);
average_PBC = cell(length(challenges),1);
average_precision = cell(length(challenges),1);
average_FMeasure = cell(length(challenges),1);
%%
recall = cell(length(challenges),repetitions);
specficity = cell(length(challenges),repetitions);
FPR = cell(length(challenges),repetitions);
FNR = cell(length(challenges),repetitions);
PBC = cell(length(challenges),repetitions);
precision = cell(length(challenges),repetitions);
FMeasure = cell(length(challenges),repetitions);
%%
TP = cell(length(challenges),repetitions);
FP = cell(length(challenges),repetitions);
FN = cell(length(challenges),repetitions);
TN = cell(length(challenges),repetitions);
SE = cell(length(challenges),repetitions);
%%
for h=1:length(challenges)
    h
    path_challenge = [path_to_database,challenges{h},'/'];
    for i=1:length(sampling_density)
        for j=1:repetitions
            path_results_challenge = [path_to_results_semi,'sampling_percentage_',...
                num2str(sampling_density(i)),'_epsilon_',num2str(epsilon),...
                '_repet_',num2str(j),'/',challenges{h},'/'];
            list_categories = dir(path_results_challenge);
            %% Metrics categories
            for k=1:length(list_categories)-2
                path_results_category = [path_results_challenge,list_categories(k+2).name,'/'];
                %load([path_results_category,'sampled_images_list.mat']);
                path_gt_category = [path_challenge,challenges{h},'/',list_categories(k+2).name,'/groundtruth/'];
                list_gt_images = dir(path_gt_category);
                %% Confusion matrix
                confusionMatrix = [0 0 0 0 0]; % TP FP FN TN SE
                for hh=1:length(list_gt_images)-2
                    ...
                    if strcmp(list_categories(k+2).name,'Candela_m1.10')
                        name_result_image = ['Candela_m1.10_',name_gt_image(indx_gt+2:indx_point(end)),'png'];
                    elseif strcmp(list_categories(k+2).name,'CaVignal')
                        name_result_image = ['CaVignal_',name_gt_image(indx_gt+2:indx_point(end)),'png'];
                    elseif strcmp(list_categories(k+2).name,'PeopleAndFoliage')
                        name_result_image = ['PeopleAndFoliage_',name_gt_image(indx_gt+2:indx_point(end)),'png'];
                    elseif strcmp(list_categories(k+2).name,'Toscana')
                        name_result_image = ['Toscana_',name_gt_image(indx_gt+2:indx_point(end)),'png'];
                    else
                        name_result_image = ['in',name_gt_image(indx_gt+2:indx_point(end)),'png'];
                    end
                    if exist([path_results_category,name_result_image]) == 2
                        im_binary = imread([path_results_category,name_result_image]);
                    else
...
save([path_to_results_metrics,'results_metrics.mat'],'recall','specficity','FPR','FNR','PBC','precision',...
    ...
    'TP','FP','FN','TN','SE','list_categories');
